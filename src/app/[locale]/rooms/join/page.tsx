...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "JoinRoomPage" });
...
export default async function JoinRoomPage({
  ...
  const t = await getTranslations("JoinRoomPage");
    ...
    <section className="flex items-center justify-center px-6 py-24">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-6 text-center text-sm text-muted">
          {t("switchPrompt")}{" "}
          <Link href="/rooms/create" className="font-medium text-primary hover:underline">
            {t("switchAction")}
          </Link>
        </p>
      </div>
